        ...
        <AnimateIn>
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-16">
            ...
            <div className="flex gap-3">
              ...
              <Link href="/v2/contact" className="text-[11px] uppercase tracking-[0.2em] border border-[var(--color-border)] px-6 py-3 text-[var(--color-text-muted)] hover:border-[var(--color-text)] hover:text-[var(--color-text)] transition-all">Contact</Link>
            </div>
          </div>
        </AnimateIn>
              ...
              <Section label="Key projects">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[
                    ...
                    <Link key={p.name} href={`/v2/work/${p.name.toLowerCase().replace(/ /g, "-").replace("maryam-ki-", "")}`} className="group bg-[var(--color-bg-card)] rounded-xl p-4 hover:bg-[var(--color-bg-card-hover)] transition-colors">
